...
setwd("C:/Users/nrasmuss/OneDrive - California Department of Water Resources/IEP/IEP_EstuaryIndicesProject/Report_Fall/WaterQualitySourceData")

#folder where data are located
data_folder <- "C:/Users/nrasmuss/OneDrive - California Department of Water Resources/IEP/IEP_EstuaryIndicesProject/Report_Fall/WaterQualitySourceData"
...
#import the two data chunks and the station info
old<-read.csv("WQ_Discrete_1975-1995.csv")
new<-read.csv("WQ_Discrete_1996-2017.csv")
siteloc<-read.csv("station_locations2.csv") #GPS coordinates of all sites


#look at column headers for all three data sets
names(old)
names(new)
#all column names match

#combine the two data chunks 
alldata<-do.call("rbind", list(old,new))
str(alldata)
...
#chlorophyll-a plots for 2019 IEP Workshop talk

#delta - no trend line
(p_chla_d_nt <- ggplot(fdt, aes(x=qyear, y=chla))+
    geom_line(colour="black", size=1.3)+geom_point(colour="black",size=3) +
    theme_iep() +
    theme(legend.position="none") + 
    scale_y_continuous("Chlorophyll-a (ug/L)",limits=c(0, max(fall$chla))) +
    scale_x_continuous("", limits=c(1974,2018)) )
#ggsave(p_chla_d_nt, file="chla_fall_delta_notrend_panel.png", path=plot_folder,scale=1.8, dpi=300, units="cm",width=8.5,height=5.4)


#delta - trend line added
(p_chla_d_tl <- ggplot(fdt, aes(x=qyear, y=chla))+
    geom_line(colour="black", size=1.3)+geom_point(colour="black",size=3) +
    theme_iep() +
    ...
    scale_y_continuous("Chlorophyll-a (ug/L)",limits=c(0, max(fall$chla))) +
    scale_x_continuous("", limits=c(1974,2018)) )
#ggsave(p_chla_d_tl, file="chla_fall_delta_trendline_panel.png", path=plot_folder,scale=1.8, dpi=300, units="cm",width=8.5,height=5.4)
